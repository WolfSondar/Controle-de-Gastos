...
async function carregarDados() {
  ...
  renderSkeletons();
  try {
    // cache: "no-store" + carimbo de tempo na URL evitam que o navegador (ou algum
    // proxy no meio do caminho) sirva uma resposta antiga em cache — é isso que
    // fazia um item já deletado "piscar" de volta na tela ao trocar de pessoa e voltar.
    const url = `${API_URL}?pessoa=${encodeURIComponent(state.pessoaAtual)}&_ts=${Date.now()}`;
...
async function dividirCompra(nome, valorTotal, categoria) {
  ...
  try {
    const [daviData, gabrielData] = await Promise.all([
      fetch(`${API_URL}?pessoa=davi&_ts=${Date.now()}`, { cache: "no-store" }).then((r) => r.json()),
      fetch(`${API_URL}?pessoa=gabriel&_ts=${Date.now()}`, { cache: "no-store" }).then((r) => r.json()),
    ]);
...
on("formDividir", "submit", async (e) => {
  ...
  if (!nome || !(valor > 0)) return;
  const categoria = categoriaDividir;
  const metade = Math.round((valor / 2) * 100) / 100;

  // A tela fecha na hora e mostra o resultado otimisticamente — quem está esperando
  // é a planilha (Apps Script), não a pessoa. A gravação real acontece em segundo plano.
  fecharAcoesConjunto();
  showToast(`Dividindo "${nome}"…`);

  if (!isAmbos()) {
    const chave = categoria === "fixos" ? "gastosFixos" : "gastosVariaveis";
    const itemOtimista = { nome, valor: metade };
    if (categoria === "fixos") itemOtimista.pago = false;
    state[chave].push(itemOtimista);
    renderAll();
  }

  const ok = await dividirCompra(nome, valor, categoria);

  if (ok) {
    showToast(`"${nome}" dividido — metade pra cada um`);
    // no modo Ambos não atualizamos localmente acima, então busca o total certo agora
    if (isAmbos()) carregarDados();
  } else {
    showToast("Não consegui dividir agora. Atualizando pra corrigir…");
    // desfaz a atualização otimista buscando os dados reais de novo
    carregarDados();
  }
});
