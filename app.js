...
function adicionarMensagemChatIA(role, text, persistir = true) {
  ...
  if (persistir) {
    ...
    lista.push({ role: role === "user" ? "user" : "assistant", text: String(text || "") });
...
function renderizarHistoricoChatIA() {
  ...
  lista.forEach((m) => adicionarMensagemChatIA(m.role, m.text, false));
}
...
async function perguntarIAChat(pergunta) {
  ...
  const data = await res.json().catch(() => null);
  if (state.pessoaAtual !== pessoaDoPedido) throw new Error("__pessoa_trocou__");
  if (!data || data.ok === false || !data.texto) throw new Error((data && data.error) || "Não consegui obter uma resposta da IA.");
