...
function calcularDuracaoAnimacaoNumero(de, para) {
  // Pequenas mudanças precisam ser suaves e legíveis; mudanças grandes
  // ganham mais tempo para dar a sensação de "arrancada" sem parecer lenta.
  const delta = Math.abs(Number(para) - Number(de));
  if (!Number.isFinite(delta) || delta <= 0) return 650;
  const duracao = 680 + Math.log10(Math.max(delta, 1)) * 430;
  return Math.max(680, Math.min(1750, duracao));
}

function animarNumero(el, de, para, duracao = null) {
  if (!el) return;
  if (de === null || de === undefined || de === para) {
    el.textContent = fmt(para);
    return;
  }
  const duracaoReal = duracao == null ? calcularDuracaoAnimacaoNumero(de, para) : duracao;
  ...
  function passo(agora) {
    const p = Math.min((agora - inicio) / duracaoReal, 1);
    // Ease-in cúbico: começa deliberadamente devagar e acelera até chegar
    // no valor final. Quanto maior a mudança, maior também é a duração.
    const acelerado = p * p * p;
    el.textContent = fmt(de + diferenca * acelerado);
...
function atualizarVisibilidadeOrigens(beneficios, ganhos) {
  const wrap = document.getElementById("saldoOrigens");
  const beneficioEl = document.querySelector(".saldo-origem-beneficio");
  const ganhoEl = document.querySelector(".saldo-origem-ganho");
  const separador = document.querySelector(".saldo-origens-separador");
  if (!wrap || !beneficioEl || !ganhoEl || !separador) return;

  const beneficioVazio = Math.abs(Number(beneficios) || 0) < 0.005;
  const ganhoVazio = Math.abs(Number(ganhos) || 0) < 0.005;
  beneficioEl.classList.toggle("is-zero", beneficioVazio);
  ganhoEl.classList.toggle("is-zero", ganhoVazio);
  separador.classList.toggle("is-hidden", beneficioVazio || ganhoVazio);
  wrap.classList.toggle("is-vazio", beneficioVazio && ganhoVazio);
}
...
function popValorFlutuante(container, delta, corFixa) {
  ...
  const span = document.createElement("span");
  span.className = "value-pop " + cor + (positivo ? " positivo" : " negativo");
  span.textContent = (positivo ? "+ " : "− ") + fmt(Math.abs(delta));
  span.setAttribute("aria-hidden", "true");
  container.appendChild(span);

  // Entrada mais perceptível: sobe um pouco, ganha opacidade e escala sem
  // ser cortada pelo card. A saída começa depois que a leitura já aconteceu.
  requestAnimationFrame(() => {
    requestAnimationFrame(() => span.classList.add("is-animating"));
  });
  const duracao = Math.max(900, Math.min(1500, 820 + Math.log10(Math.max(Math.abs(delta), 1)) * 260));
  setTimeout(() => span.classList.add("is-leaving"), duracao * 0.58);
  setTimeout(() => span.remove(), duracao + 420);
}
...
function renderTotais() {
  ...
  atualizarVisibilidadeOrigens(ganhosPorOrigem.beneficios, ganhosPorOrigem.ganhos);
