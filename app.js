...
function posicionarMenuIcone(picker) {
  ...
  // EDIÇÃO: o menu fica ancorado ao próprio campo, dentro do modal.
  // Isso evita que o seletor apareça deslocado no topo da tela.
  if (picker.dataset.pickerFor === "editar") {
    const margem = 12;
    const viewportH = window.innerHeight;
    const espacoAbaixo = viewportH - rect.bottom - margem;
    const espacoAcima = rect.top - margem;
    const alturaDesejada = Math.min(menu.scrollHeight || 260, 300);
    const abrirAcima = espacoAbaixo < Math.min(alturaDesejada, 220) && espacoAcima > espacoAbaixo;
    const alturaDisponivel = abrirAcima ? espacoAcima - 7 : espacoAbaixo - 7;

    menu.style.position = "absolute";
    menu.style.width = "100%";
    menu.style.left = "0";
    menu.style.right = "auto";
    menu.style.top = abrirAcima ? "auto" : "calc(100% + 7px)";
    menu.style.bottom = abrirAcima ? "calc(100% + 7px)" : "auto";
    menu.style.maxHeight = `${Math.max(150, Math.min(300, alturaDisponivel))}px`;
    menu.classList.toggle("is-above", abrirAcima);
    return;
  }
  ...
  // Mede a altura real depois de tornar o menu visível, mas antes da posição
  // final. Assim decidimos automaticamente se ele cabe abaixo ou acima.
  const altura = Math.min(menu.scrollHeight || 270, viewportH - margem * 2);
  const espacoAbaixo = viewportH - rect.bottom - margem;
  const espacoAcima = rect.top - margem;
  const abrirAcima = espacoAbaixo < Math.min(altura, 270) && espacoAcima > espacoAbaixo;
  ...
  if (abrirAcima) {
    ...
    top = Math.min(viewportH - altura - margem, rect.bottom + gap);
    top = Math.max(margem, top);
    menu.classList.remove("is-above");
  }

  // Alinha com o trigger, mas garante que a caixa nunca saia lateralmente.
  let left = rect.left;
  if (left + largura > viewportW - margem) left = viewportW - largura - margem;
  left = Math.max(margem, left);

  menu.style.left = `${left}px`;
  menu.style.top = `${top}px`;
  menu.style.maxHeight = `${Math.max(120, viewportH - margem * 2)}px`;
}
...
function montarInfoPrazoCaixinha(data, completa) {
  ...
  return `<span class="caixinha-prazo ${classe}">${calendario}<span>${texto}</span></span>`;
}
