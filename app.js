...
function renderOpcoesIconesCaixinhas(picker) {
  ...
  menu.innerHTML = `
    <div class="caixinha-icon-picker-search-wrap">
      <svg class="caixinha-icon-picker-search-icon" viewBox="0 0 24 24" fill="none" aria-hidden="true">
  ...
  const desenhar = (termo = "") => {
    ...
    const filtradas = opcoes.filter(({ nome, label }) => {
      if (!busca) return true;
      if (!nome) return "sem icone sem ícone".includes(busca);
      return normalizarTextoBuscaIcone(`${nome} ${label}`).includes(busca);
    });
