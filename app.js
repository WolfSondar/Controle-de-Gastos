...
// ---------------------------------------------------------------------
const IA_CHAT_STORAGE_PREFIX = "caixaChatHistorico:";
...
function criarCardRespostaIA(card, index) {
  ...
  el.innerHTML = `
    <div class="ia-card-top">
      <div class="ia-card-index">${String(index + 1).padStart(2, "0")}</div>
  ...
  if (tipo === "chart") {
    ...
    card.innerHTML = `
      <div class="ia-chart-head">
        ...
        ${chart.subtitle ? `<span>${escapeHtml(chart.subtitle)}</span>` : ""}
      </div>
...
function renderizarHistoricoChatIA() {
  ...
  if (!lista.length) {
    adicionarMensagemChatIA("assistant", isAmbos()
      ...
      : `Oi! Estou olhando os dados do ${PESSOA_LABEL[state.pessoaAtual] || "seu"} Caixa. Pode me perguntar se uma compra cabe, como está uma meta, quanto falta guardar ou qualquer outra coisa sobre suas finanças.`);
    return;
  }
  lista.forEach((m) => {
...
function atualizarContextoChatIA() {
  const el = document.getElementById("iaChatContext");
  if (!el) return;
  const nome = isAmbos() ? "Modo Juntos · Davi + Gabriel" : `Modo ${PESSOA_LABEL[state.pessoaAtual] || "pessoal"}`;
  const mes = state.mesAtual && state.anoAtual ? `${MESES_LABEL[state.mesAtual - 1]}/${state.anoAtual}` : "mês atual";
  el.textContent = `${nome} · dados de ${mes} · a IA usa os dados atuais da planilha e suas configurações pessoais.`;
}
