...
function renderPessoaSwitch() {
  ...
  if (typeof iaChatAberto !== "undefined" && iaChatAberto) atualizarContextoChatIA();
}
...
function ehBeneficioAlimentacaoDavi(nome, pessoa) {
  if (pessoa !== "davi") return false;
  const normalizado = String(nome || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
  // Qualquer ganho cujo nome contenha "beneficio" (ex.: Multibenefícios)
  // é saldo de benefício alimentar do Davi, não dinheiro livre.
  return normalizado.indexOf("beneficio") !== -1;
}
...
function lancamentosComNomeDoMes() {
  const ganhos = (state.ganhos || [])
    ...
    .map((g) => {
      const pessoaCodigo = g.pessoa || state.pessoaAtual;
      const beneficio = ehBeneficioAlimentacaoDavi(g.nome, pessoaCodigo);
      const det = {
        nome: g.nome,
        valor: Number(g.valor) || 0,
        data: g.data || "",
        tipoGanho: beneficio ? "beneficio_alimentacao" : "saldo_livre"
      };
      if (g.pessoa) det.pessoa = PESSOA_LABEL[g.pessoa] || g.pessoa;
      return det;
    });
  const gastosFixos = (state.gastosFixos || [])
    .map((g) => detalheItemParaInsight(g, "fixo"));
...
// Recorte exato dos últimos 7 dias com os lançamentos que já foram recebidos/pagos.
// O estado do mês atual é a fonte de nomes e datas; não inventamos dados de um
// mês anterior que não estejam carregados no app.
function lancamentosRecebidosPagosUltimos7Dias() {
  const hoje = new Date();
  const hojeSemHora = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());
  const inicio = new Date(hojeSemHora);
  inicio.setDate(inicio.getDate() - 6);

  function dentroDosUltimos7Dias(data) {
    const m = /^(\d{4})-(\d{2})-(\d{2})/.exec(String(data || ""));
    if (!m) return false;
    const d = new Date(Number(m[1]), Number(m[2]) - 1, Number(m[3]));
    return d >= inicio && d <= hojeSemHora;
  }

  const ganhos = (state.ganhos || [])
    .filter((g) => g.recebido === true && dentroDosUltimos7Dias(g.data))
    .map((g) => ({
      nome: g.nome,
      valor: Number(g.valor) || 0,
      tipo: "ganho",
      categoria: ehBeneficioAlimentacaoDavi(g.nome, g.pessoa || state.pessoaAtual) ? "Benefício alimentação" : "Recebido",
      tipoGanho: ehBeneficioAlimentacaoDavi(g.nome, g.pessoa || state.pessoaAtual) ? "beneficio_alimentacao" : "saldo_livre",
      data: g.data || "",
      status: "recebido",
      ...(g.pessoa ? { pessoa: PESSOA_LABEL[g.pessoa] || g.pessoa } : {})
    }));

  const gastosFixos = (state.gastosFixos || [])
    .filter((g) => g.pago === true && dentroDosUltimos7Dias(g.data))
    .filter((g) => !ehLancamentoDeCaixinha(g.nome))
    .map((g) => ({
      nome: g.nome,
      valor: Number(g.valor) || 0,
      tipo: "gasto",
      categoria: (g.tipo && String(g.tipo).trim()) || "Outros",
      data: g.data || "",
      status: "pago",
      ...(g.pessoa ? { pessoa: PESSOA_LABEL[g.pessoa] || g.pessoa } : {})
    }));
  ...
  const gastosVariaveis = (state.gastosVariaveis || [])
    .filter((g) => g.pago === true && g.lembrete !== true && dentroDosUltimos7Dias(g.data))
    .filter((g) => !ehLancamentoDeCaixinha(g.nome))
    .map((g) => ({
      nome: g.nome,
      valor: Number(g.valor) || 0,
  ...
  return ganhos.concat(gastosFixos, gastosVariaveis)
    .sort((a, b) => String(b.data || "").localeCompare(String(a.data || "")) || (a.tipo === "gasto" ? -1 : 1));
}
...
function montarResumoParaInsight() {
  ...
  const ganhosBeneficioAlimentacao = state.pessoaAtual === "davi"
    ? (state.ganhos || []).reduce((acc, g) => acc + (g.recebido === true && ehBeneficioAlimentacaoDavi(g.nome, "davi") ? (Number(g.valor) || 0) : 0), 0)
    : 0;
  const gastosAlimentacaoPagos = (state.gastosFixos || []).filter(fixoEhPago).concat((state.gastosVariaveis || []).filter(variavelContaNoSaldo)).reduce((acc, g) => {
    const cat = (g.tipo && String(g.tipo).trim()) || "Outros";
    return acc + (cat === "Alimentação" ? (Number(g.valor) || 0) : 0);
  }, 0);
  // No Davi, o benefício é um saldo separado, restrito a alimentação.
  // Os gastos de Alimentação consomem esse saldo primeiro; o excedente
  // fica por conta do saldo livre.
  const saldoBeneficioAlimentacaoDavi = state.pessoaAtual === "davi"
    ? Math.max(0, ganhosBeneficioAlimentacao - gastosAlimentacaoPagos)
    : 0;
  const saldoTotalAgora = ganhosRecebidos - gastosFixosPagos - gastosVariaveisPagos;
  const saldoLivreAgora = state.pessoaAtual === "davi"
    ? saldoTotalAgora - saldoBeneficioAlimentacaoDavi
    : saldoTotalAgora;
  ...
  const resumo = {
    ...
    mesAtual: {
      ...
      saldoDisponivelAgora: saldoTotalAgora,
      saldoLivreAgora,
      ganhosBeneficioAlimentacaoDavi: ganhosBeneficioAlimentacao,
      saldoBeneficioAlimentacaoDavi,
      gastosAlimentacaoPagos,
      regraBeneficio: state.pessoaAtual === "davi"
        ? "Ganho com nome contendo a palavra benefício/beneficio (ex.: Multibenefícios) é benefício alimentar e NÃO é saldo livre. Ele pode ser usado para alimentação; para outras compras, use saldoLivreAgora. No Gabriel, ganhos com benefício entram normalmente no saldo livre."
        : state.pessoaAtual === "gabriel"
          ? "No Gabriel, ganhos com a palavra benefício/beneficio entram normalmente como saldo livre, inclusive Multibenefícios."
          : "No modo Juntos, o tratamento de benefício depende da pessoa: Davi = benefício alimentar separado; Gabriel = saldo livre.",
      guardadoNoMes,
    ...
    lancamentosRecebidosPagosUltimos7Dias: lancamentosRecebidosPagosUltimos7Dias(),
  };
...
// ---------------------------------------------------------------------
// ASSISTENTE FINANCEIRO IA — chat livre, personalizado por pessoa/modo
// ---------------------------------------------------------------------
const IA_CHAT_STORAGE_PREFIX = "caixaChatHistorico:v2:";
let iaChatAberto = false;
let iaChatEnviando = false;

function iaChatStorageKey() { return IA_CHAT_STORAGE_PREFIX + state.pessoaAtual; }
function carregarHistoricoChatIA() {
  try {
    const raw = sessionStorage.getItem(iaChatStorageKey());
    const lista = raw ? JSON.parse(raw) : [];
    return Array.isArray(lista) ? lista.slice(-12) : [];
  } catch (_err) { return []; }
}
function salvarHistoricoChatIA(lista) {
  try { sessionStorage.setItem(iaChatStorageKey(), JSON.stringify((lista || []).slice(-12))); } catch (_err) {}
}
function formatarHoraChatIA() {
  return new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });
}
function renderizarValorChat(texto) {
  const seguro = escapeHtml(String(texto || ""));
  return seguro
    .replace(/\{\{ganho:([^{}]+)\}\}/gi, '<span class="ia-chat-valor ia-chat-ganho">$1</span>')
    .replace(/\{\{gasto:([^{}]+)\}\}/gi, '<span class="ia-chat-valor ia-chat-gasto">$1</span>')
    .replace(/\{\{guardado:([^{}]+)\}\}/gi, '<span class="ia-chat-valor ia-chat-guardado">$1</span>')
    .replace(/\{\{rendimento:([^{}]+)\}\}/gi, '<span class="ia-chat-valor ia-chat-rendimento">$1</span>')
    .replace(/\{\{\+([^{}]+)\}\}/g, '<span class="ia-chat-valor ia-chat-positivo">$1</span>')
    .replace(/\{\{-([^{}]+)\}\}/g, '<span class="ia-chat-valor ia-chat-negativo">$1</span>')
    .replace(/\{\{([^{}]+)\}\}/g, "$1")
    .replace(/\n/g, "<br>");
}

function formatarValorCardIA(valor) {
  return renderizarValorChat(valor || "");
}

function formatarDataCardIA(data) {
...
function criarCardRespostaIA(card, index, compacto = false) {
  const type = ["ganho", "gasto", "guardado", "rendimento", "neutro"].includes(card?.type) ? card.type : "neutro";
  const el = document.createElement("article");
  el.className = `ia-response-card ia-response-card-${type}${compacto ? " ia-transaction-row" : ""}`;
  const meta = [
    card?.category ? `<span>${escapeHtml(card.category)}</span>` : "",
    card?.date ? `<span>${formatarDataCardIA(card.date)}</span>` : "",
    card?.status ? `<span>${escapeHtml(card.status)}</span>` : ""
  ].filter(Boolean).join('<span class="ia-card-meta-sep">•</span>');

  el.innerHTML = `
    <div class="ia-card-top">
      <span class="ia-card-dot" aria-hidden="true"></span>
      <div class="ia-card-main">
        <strong class="ia-card-title">${escapeHtml(card?.title || "Item")}</strong>
        ${card?.subtitle ? `<span class="ia-card-subtitle">${escapeHtml(card.subtitle)}</span>` : ""}
      </div>
      <div class="ia-card-value">${formatarValorCardIA(card?.value || "")}</div>
    </div>
    ${meta ? `<div class="ia-card-meta">${meta}</div>` : ""}
  `;
  return el;
}

function cardsParecemTransacoesIA(cards) {
  return Array.isArray(cards) && cards.length > 0 && cards.every((card) =>
    card && (card.type === "ganho" || card.type === "gasto") && card.date && card.status
  );
}

function criarRespostaIA(resposta) {
  const wrap = document.createElement("div");
  wrap.className = "ia-response";
  const tipo = resposta?.tipo || "text";

  if (tipo === "cards") {
    const cards = Array.isArray(resposta.cards) ? resposta.cards : [];
    const grid = document.createElement("div");
    grid.className = `ia-cards-grid${cardsParecemTransacoesIA(cards) ? " ia-transaction-list" : ""}`;
    cards.forEach((card, i) => {
      grid.appendChild(criarCardRespostaIA(card, i, cardsParecemTransacoesIA(cards)));
    });
  ...
  if (tipo === "poll") {
    const poll = resposta.poll || {};
    const box = document.createElement("div");
    box.className = "ia-poll-card";
    box.innerHTML = `
      <div class="ia-poll-question">${renderizarValorChat(poll.question || "Qual opção você prefere?")}</div>
      <div class="ia-poll-options"></div>
    `;
    const options = box.querySelector(".ia-poll-options");
    (Array.isArray(poll.options) ? poll.options : []).forEach((option) => {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "ia-poll-option";
      btn.textContent = String(option);
      btn.addEventListener("click", () => {
        if (iaChatEnviando) return;
        box.querySelectorAll(".ia-poll-option").forEach((b) => b.disabled = true);
        if (iaChatInput) iaChatInput.value = String(option);
  ...
  if (tipo === "chart") {
    const chart = resposta.chart || {};
    const segmentos = Array.isArray(chart.segments) ? chart.segments : [];
    const total = segmentos.reduce((acc, seg) => acc + Math.max(0, Number(seg.value) || 0), 0);
    const card = document.createElement("div");
    card.className = "ia-chart-card";
    card.innerHTML = `
      <div class="ia-chart-head">
        <strong>${escapeHtml(chart.title || "Resumo")}</strong>
        <b class="ia-chart-total">${escapeHtml(fmt(total))}</b>
      </div>
      <div class="ia-segmented-bar" role="img" aria-label="${escapeHtml(chart.title || "Gráfico")}" ></div>
      <div class="ia-chart-legend"></div>
    `;
    const bar = card.querySelector(".ia-segmented-bar");
    const legend = card.querySelector(".ia-chart-legend");
    segmentos.forEach((seg, i) => {
      const valor = Math.max(0, Number(seg.value) || 0);
      const pct = total > 0 ? (valor / total) * 100 : 0;
      const cor = typeof corDaCategoria === "function"
        ? corDaCategoria(String(seg.label || "Outros"), i)
        : (Array.isArray(PALETA_CATEGORIAS) ? PALETA_CATEGORIAS[i % PALETA_CATEGORIAS.length] : "var(--gold)");
      const part = document.createElement("div");
      part.className = "ia-segment";
      part.style.width = `${pct}%`;
      part.style.background = cor;
      part.title = `${seg.label}: ${fmt(valor)} (${pct.toFixed(1).replace(".", ",")}%)`;
      bar.appendChild(part);

      const row = document.createElement("div");
      row.className = "ia-chart-row";
      row.innerHTML = `
        <span class="ia-chart-dot" style="background:${cor}"></span>
        <span class="ia-chart-label">${escapeHtml(seg.label || "Outros")}</span>
        <strong>${escapeHtml(fmt(valor))}</strong>
        <span class="ia-chart-pct">${pct.toFixed(1).replace(".", ",")}%</span>
      `;
  ...
  const texto = String(resposta?.texto || "");
  const textoEl = document.createElement("div");
  textoEl.className = "ia-response-text";
  textoEl.innerHTML = renderizarValorChat(texto);
  wrap.appendChild(textoEl);
  return wrap;
}

function normalizarRespostaIA(data) {
  if (data && data.resposta && typeof data.resposta === "object") return data.resposta;
  if (data && data.texto) return { tipo: "text", texto: String(data.texto) };
  return { tipo: "text", texto: "Não consegui interpretar a resposta da IA." };
}

function adicionarMensagemChatIA(role, content, persistir = true) {
  const wrap = document.getElementById("iaChatMessages");
  if (!wrap) return;
  const item = document.createElement("div");
  item.className = `ia-chat-message ${role === "user" ? "user" : "assistant"}`;
  ...
  if (role === "assistant" && content && typeof content === "object") {
    resposta = content;
    bubble.appendChild(criarRespostaIA(resposta));
  } else {
    const text = String(content || "");
    if (role === "assistant") {
      bubble.innerHTML = renderizarValorChat(text);
    } else {
  ...
  hora.className = "ia-chat-time";
  hora.textContent = formatarHoraChatIA();
  ...
  if (persistir) {
    const lista = carregarHistoricoChatIA();
    const registro = role === "assistant"
      ? { role: "assistant", resposta: resposta || { tipo: "text", texto: String(content || "") } }
      : { role: "user", text: String(content || "") };
    lista.push(registro);
    salvarHistoricoChatIA(lista);
  }
}

function renderizarHistoricoChatIA() {
  const wrap = document.getElementById("iaChatMessages");
  if (!wrap) return;
  wrap.innerHTML = "";
  const lista = carregarHistoricoChatIA();
  if (!lista.length) {
    adicionarMensagemChatIA("assistant", isAmbos()
      ? "Oi! Estou olhando as finanças combinadas de vocês. Pode me perguntar sobre gastos, saldo, caixinhas, metas, prazos ou uma compra que você esteja pensando em fazer."
      : "Oi! Estou aqui para ajudar com suas finanças. Pode me perguntar sobre gastos, saldo, caixinhas, metas ou se uma compra cabe no seu orçamento.");
    return;
  }
  lista.forEach((m) => {
    if (m.role === "assistant" && m.resposta) adicionarMensagemChatIA("assistant", m.resposta, false);
    else adicionarMensagemChatIA(m.role, m.text, false);
  });
}
function atualizarContextoChatIA() {}
function abrirChatIA() {
  const backdrop = document.getElementById("iaChatBackdrop");
  if (!backdrop) return;
  iaChatAberto = true;
  atualizarContextoChatIA();
  renderizarHistoricoChatIA();
  backdrop.classList.remove("is-hidden");
  backdrop.setAttribute("aria-hidden", "false");
  document.body.classList.add("ia-chat-aberto");
  setTimeout(() => document.getElementById("iaChatInput")?.focus(), 80);
}
function fecharChatIA() {
  ...
  document.body.classList.remove("ia-chat-aberto");
}
function mostrarDigitandoChatIA() {
  const wrap = document.getElementById("iaChatMessages");
  if (!wrap) return;
  const item = document.createElement("div");
  item.className = "ia-chat-message assistant";
  item.id = "iaChatTyping";
  const bubble = document.createElement("div");
  bubble.className = "ia-chat-bubble";
  bubble.innerHTML = '<span class="ia-chat-typing"><span></span><span></span><span></span></span>';
  item.appendChild(bubble);
  wrap.appendChild(item);
  wrap.scrollTop = wrap.scrollHeight;
}
function removerDigitandoChatIA() { document.getElementById("iaChatTyping")?.remove(); }

async function perguntarIAChat(pergunta) {
  if (!API_URL || API_URL.includes("COLE_AQUI")) throw new Error("Configure a URL do Apps Script em config.js.");
  if (!navigator.onLine) throw new Error("Você está offline. Conecte-se à internet para conversar com a IA.");
  const pessoaDoPedido = state.pessoaAtual;
  if (!state.historico) {
    await carregarHistorico();
    if (state.pessoaAtual !== pessoaDoPedido) throw new Error("__pessoa_trocou__");
  ...
  const resumo = montarResumoParaInsight();
  resumo.dataHoje = dataHojeISO();
  resumo.modoAtual = pessoaDoPedido;
  const historico = carregarHistoricoChatIA().slice(-10);
  const res = await fetch(API_URL, {
    method: "POST",
    body: JSON.stringify({
      action: "chatIA",
      pessoa: pessoaDoPedido,
      pergunta: pergunta,
      historico: historico,
      resumo: resumo,
    }),
  ...
  const data = await res.json().catch(() => null);
  if (state.pessoaAtual !== pessoaDoPedido) throw new Error("__pessoa_trocou__");
  if (!data || data.ok === false || (!data.resposta && !data.texto)) {
    throw new Error((data && data.error) || "Não consegui obter uma resposta da IA.");
  }
  return normalizarRespostaIA(data);
}

on("iaFab", "click", abrirChatIA);
on("iaChatClose", "click", fecharChatIA);
const iaChatBackdropEl = document.getElementById("iaChatBackdrop");
if (iaChatBackdropEl) {
  iaChatBackdropEl.addEventListener("click", (e) => {
    if (e.target === iaChatBackdropEl) fecharChatIA();
  });
}
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && iaChatAberto) fecharChatIA();
});
const iaChatForm = document.getElementById("iaChatForm");
const iaChatInput = document.getElementById("iaChatInput");
const iaChatSend = document.getElementById("iaChatSend");
if (iaChatInput) {
  iaChatInput.addEventListener("input", () => {
    iaChatInput.style.height = "auto";
    iaChatInput.style.height = Math.min(120, iaChatInput.scrollHeight) + "px";
  });
  iaChatInput.addEventListener("keydown", (e) => {
...
if (iaChatForm) {
  iaChatForm.addEventListener("submit", async (e) => {
    e.preventDefault();
    if (iaChatEnviando) return;
    const pergunta = String(iaChatInput?.value || "").trim();
    if (!pergunta) return;
    iaChatEnviando = true;
    if (iaChatSend) iaChatSend.disabled = true;
    if (iaChatInput) { iaChatInput.value = ""; iaChatInput.style.height = "auto"; }
    adicionarMensagemChatIA("user", pergunta);
    mostrarDigitandoChatIA();
    try {
      const resposta = await perguntarIAChat(pergunta);
      removerDigitandoChatIA();
      adicionarMensagemChatIA("assistant", resposta);
    } catch (err) {
      removerDigitandoChatIA();
      if (err && err.message === "__pessoa_trocou__") return;
      adicionarMensagemChatIA("assistant", `Não consegui responder agora. ${err?.message || "Tente novamente em instantes."}`);
    } finally {
      iaChatEnviando = false;
      if (iaChatSend) iaChatSend.disabled = false;
      if (iaChatInput) iaChatInput.focus();
    }
  });
}
