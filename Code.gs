...
function gerarInsightComGemini(pessoa, periodo, resumo) {
  try {
    ...
    const regrasComuns = [
      ...
      "O resumo também traz lancamentosComNomeDoMesAtual.ganhosDoMes e .gastosVariaveisDoMes, com o NOME DE VERDADE que a pessoa deu a cada lançamento desse mês (ex: 'Almoço - Tia Marina', 'Uber - Aeroporto'). Preste atenção nesses nomes: se um deles deixar claro de onde veio o dinheiro ou pra onde foi (uma pessoa, um lugar, uma ocasião), pode citar isso literalmente em um dos insights pra ficar mais pessoal e específico (ex: comentar que o dinheiro do almoço veio da Tia Marina) — mas só quando o nome realmente disser isso com clareza, nunca invente uma relação ou um contexto que o nome não deixa explícito, e não force esse ângulo em todo insight.",
