...
function gerarChatComGemini(pessoa, pergunta, historico, resumo) {
  try {
    ...
    const historicoSeguro = Array.isArray(historico) ? historico.slice(-10).map(function (m) {
      return { role: m && m.role === "assistant" ? "assistant" : "user", text: String((m && m.text) || "").slice(0, 1500) };
    }) : [];

    const regras = [
      "Você é o assistente financeiro pessoal dentro do app Caixa. Responda em português do Brasil, de forma humana, clara e específica.",
    ...
    const historicoTexto = historicoSeguro.length
      ? historicoSeguro.map(function (m) { return (m.role === "user" ? "Usuário: " : "Assistente: ") + m.text; }).join("\n")
