...
function gerarChatComGemini(pessoa, pergunta, historico, resumo) {
  try {
    ...
    const regras = [
      ...
      "No cards, se a pergunta for sobre transações recentes, priorize lancamentosComNomeDoMesAtual.ganhosDoMes e gastosDoMes e mostre nome, valor, categoria, data/status quando disponíveis. Não transforme todos os lançamentos em um texto corrido.",
    ...
    if (resposta.tipo === "cards") {
      resposta.cards = Array.isArray(resposta.cards) ? resposta.cards.slice(0, 6).map(function (c) {
        return {
