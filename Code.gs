 ...
 *   U = DATA (prazo opcional da caixinha, formato yyyy-MM-dd)
 *   V = ICON (nome do arquivo do ícone personalizado, ex: caixa_zelda.png)
 *
...
const COL_DATA_CAIXINHA = 21; // U — prazo opcional da caixinha
const COL_ICONE = 22; // V — ícone personalizado da caixinha
...
function doPost(e) {
  try {
    ...
    if (action === "chatIA") {
      return respond(gerarChatComGemini(body.pessoa, body.pergunta, body.historico, body.resumo));
    }
...
// A API do Gemini pode devolver 429/503 em picos temporários de demanda.
// O app usa algumas tentativas com espera crescente e, se necessário,
// troca para o modelo de fallback antes de desistir.
function chamarGeminiComRetry(corpo, apiKey) {
  const modelos = [GEMINI_MODEL, GEMINI_FALLBACK_MODEL];
  const esperasMs = [1200, 2500, 5000];
  let ultimoStatus = 0;
  let ultimoData = {};
  ...
  for (let m = 0; m < modelos.length; m++) {
    ...
    for (let tentativa = 0; tentativa < esperasMs.length + 1; tentativa++) {
      if (tentativa > 0) {
        // Pequeno jitter evita que várias requisições voltem juntas.
        const jitter = Math.floor(Math.random() * 350);
        Utilities.sleep(esperasMs[tentativa - 1] + jitter);
      }
      ...
      try {
        ...
        ultimoStatus = 0;
        ultimoData = { error: { message: String(fetchErr) } };
        if (tentativa < esperasMs.length) continue;
        break;
      }

      ultimoStatus = res.getResponseCode();
      try {
        ultimoData = JSON.parse(res.getContentText() || "{}");
      } catch (parseErr) {
        ultimoData = { error: { message: res.getContentText() || "Resposta inválida do Gemini." } };
      }

      if (ultimoStatus >= 200 && ultimoStatus < 300) {
        return { ok: true, status: ultimoStatus, data: ultimoData, modelo: modelo };
      }

      // Só repetimos erros transitórios. 400/401/403/404, por exemplo,
      // normalmente indicam configuração ou requisição inválida e não
      // melhoram esperando.
      const transitorio = ultimoStatus === 408 || ultimoStatus === 429 || ultimoStatus >= 500;
...
function mensagemAmigavelGemini(status, data) {
  const bruto = String((data && data.error && data.error.message) || "").toLowerCase();

  if (status === 401 || status === 403 || bruto.indexOf("api key") >= 0 || bruto.indexOf("permission") >= 0) {
    return "A IA está sem autorização para responder. Confira a chave GEMINI_API_KEY nas propriedades do script.";
  }
  if (status === 429) {
    return "A IA está recebendo muitas solicitações agora. Tente novamente em alguns segundos.";
  }
  if (status === 408 || status >= 500 || bruto.indexOf("high demand") >= 0 || bruto.indexOf("overloaded") >= 0 || bruto.indexOf("unavailable") >= 0) {
    return "A IA ficou temporariamente sobrecarregada. Eu já tentei novamente e também um modelo de reserva, mas não consegui uma resposta agora. Tente de novo em instantes.";
  }
  return "Não consegui obter uma resposta da IA agora. Tente novamente em instantes.";
}
...
function gerarInsightComGemini(pessoa, periodo, resumo) {
  try {
    ...
    const respostaGemini = chamarGeminiComRetry(corpo, apiKey);
    if (!respostaGemini.ok) {
      return { ok: false, error: mensagemAmigavelGemini(respostaGemini.status, respostaGemini.data), code: "temporarily_unavailable" };
    }
    const data = respostaGemini.data;
...
// ---------------------------------------------------------------------
// CHAT LIVRE COM IA — usa o mesmo contexto pessoal do insight, mas responde
// uma pergunta por vez com os dados financeiros atuais do usuário.
// ---------------------------------------------------------------------
function gerarChatComGemini(pessoa, pergunta, historico, resumo) {
  try {
    const apiKey = PropertiesService.getScriptProperties().getProperty(GEMINI_API_KEY_PROPRIEDADE);
    if (!apiKey) {
      return { ok: false, error: "Chave do Gemini não configurada. Configure GEMINI_API_KEY nas propriedades do script." };
    }

    const pessoaCodigo = String(pessoa || "davi").toLowerCase();
    const ehCasal = pessoaCodigo === "ambos";
    const nomePessoa = PESSOA_NOME[pessoaCodigo] || "o usuário";
    const perguntaLimpa = String(pergunta || "").trim();
    if (!perguntaLimpa) return { ok: false, error: "Digite uma pergunta." };

    const imersao = textoImersaoIA(pessoaCodigo);
    const tom = textoTomIA(pessoaCodigo);
    const historicoSeguro = Array.isArray(historico) ? historico.slice(-10).map(function (m) {
      const role = m && m.role === "assistant" ? "assistant" : "user";
      let text = "";
      if (role === "assistant" && m && m.resposta) {
        try { text = JSON.stringify(m.resposta); } catch (_e) { text = String(m.text || ""); }
      } else {
        text = String((m && m.text) || "").slice(0, 1500);
      }
      return { role: role, text: text.slice(0, 2500) };
    }) : [];

    const regras = [
      "Você é o assistente financeiro pessoal dentro do app Caixa. Responda em português do Brasil, de forma humana, clara, específica e MUITO CONCISA.",
      "Você NÃO executa ações, não altera lançamentos, não cria compras, não transfere dinheiro e não muda a planilha. Você apenas analisa os dados fornecidos e conversa com o usuário.",
      "Use somente os dados do resumo fornecido. Não invente renda, gasto, saldo, prazo, categoria, caixinha ou compromisso que não esteja no resumo.",
      "Quando a pessoa perguntar se uma compra cabe, faça a conta com os números disponíveis. Diferencie saldo disponível agora de dinheiro guardado nas caixinhas. Considere pendências do mês e metas com prazo quando forem relevantes. Se houver incerteza porque algum dado não existe, diga isso claramente em vez de chutar.",
      "REGRA DE BENEFÍCIO: no Davi, qualquer ganho cujo nome contenha a palavra 'benefício' ou 'beneficio' — inclusive nomes como 'Multibenefícios' — NÃO é saldo livre. É benefício restrito a alimentação. Para uma compra de comida/alimentação, você pode considerar saldoBeneficioAlimentacaoDavi; para qualquer compra que não seja alimentação, use saldoLivreAgora e NÃO conte o benefício. O resumo também informa ganhosBeneficioAlimentacaoDavi, gastosAlimentacaoPagos e a regraBeneficio. No Gabriel, mesmo que o nome contenha 'benefício/beneficio' ou 'Multibenefícios', o valor entra normalmente no saldo livre e pode ser usado para qualquer tipo de compra. No modo Juntos, respeite a pessoa do lançamento e não misture os saldos.",
      "Para caixinhas, valorGuardado é o total atual real. guardadoNesseMes é somente o que entrou neste mês. Nunca some os dois. Se houver meta e prazo, use faltaParaMeta, mesesAtePrazo, diasAtePrazo e necessarioGuardarPorMes quando disponíveis.",
      "Quando falar de prazo, use a data de hoje e os campos calculados. Não trate um mês em andamento como se estivesse fechado.",
      "Pode sugerir caminhos práticos e fazer contas, mas não seja alarmista. Se uma compra parece apertada, explique por quê e, quando possível, diga quanto sobraria ou quanto faltaria.",
      "Se o usuário perguntar algo que não seja financeiro, responda brevemente que este assistente foi feito para ajudar com as finanças do Caixa e tente redirecionar para o contexto financeiro.",
      ehCasal
        ? "Você está no modo Juntos. O resumo representa as finanças combinadas de Davi e Gabriel. Quando relevante, compare ou separe os dois usando os campos porPessoa e o campo pessoa dos lançamentos. Nunca atribua um gasto a uma pessoa sem o resumo confirmar isso."
        : "Você está no modo individual de " + nomePessoa + ". Responda diretamente para essa pessoa.",
      "Se houver contexto pessoal na imersão abaixo, use-o de forma natural para deixar a resposta personalizada, sem revelar que recebeu uma ficha interna.",
      "Não diga que você 'viu a planilha' ou que acessou dados externos. Diga simplesmente 'pelos seus números' ou equivalente.",
      "FORMATAÇÃO DE VALORES: sempre que citar dinheiro, use o marcador correto exatamente assim: {{ganho:R$ 5,00}} para ganho; {{gasto:R$ 5,00}} para gasto; {{guardado:R$ 5,00}} para dinheiro em caixinha/meta; {{rendimento:R$ 5,00}} para rendimento. Para saldo genérico, use {{+R$ 5,00}} ou {{-R$ 5,00}}. Nunca escreva valor em reais sem marcador e nunca abrevie valores.",
      "Não use markdown. Não use tabelas. Não escreva introduções longas, despedidas ou explicações sobre o formato.",
      "IMPORTANTE: escolha UMA forma de resposta entre quatro formatos: text, cards, poll ou chart. Escolha o formato que melhor combina com a pergunta. Não misture formatos na mesma resposta.",
      "FORMATO text: use para perguntas que pedem conselho, análise ou resposta direta. A resposta deve ter NO MÁXIMO 3 parágrafos curtos e, de preferência, 1 só. Seja objetiva.",
      "FORMATO cards: use quando a pergunta pede uma lista, histórico, transações recentes, gastos/ganhos, caixinhas ou vários itens que ficam mais fáceis de escanear. Gere um card por item, sem agrupar e sem transformar a lista em texto corrido; para transações recentes, pode usar até 24 cards. Cada card deve ter title, value (com marcador {{...}}), type (ganho|gasto|guardado|rendimento|neutro), e opcionalmente date, category, status, subtitle.",
      "FORMATO poll: use quando a pergunta estiver ambígua ou quando houver duas ou mais interpretações/caminhos relevantes e você realmente precisar da escolha do usuário. Faça uma pergunta curta e de 2 a 4 opções. Não use poll só por diversão.",
      "FORMATO chart: use para perguntas comparativas ou de ranking em que uma barra segmentada ajude, especialmente 'qual categoria gastei mais?', distribuição por categoria, participação dos gastos ou comparação de vários itens. O gráfico deve ser ENXUTO: title curto, sem texto explicativo e sem subtitle. NÃO coloque o total geral no título nem diga quanto foi gasto se o usuário perguntou apenas qual categoria ficou em primeiro; o frontend mostra a discriminação de cada categoria com valor e porcentagem abaixo da barra. Os segmentos devem conter somente dados que existam no resumo. Use label e value numérico exato; o frontend calcula as porcentagens.",
      "No chart, NÃO invente categorias. Para 'gastos por categoria', use os valores de categorias do mês atual presentes no resumo. Se a pergunta for sobre outro período, use o bloco correspondente do resumo.",
      "No cards, se a pergunta for sobre transações recentes (ex.: 'me fale sobre transações recentes'), USE lancamentosRecebidosPagosUltimos7Dias e mostre CADA gasto que já foi pago e CADA ganho que já foi recebido nesse recorte, um card por transação, ordenado da mais recente para a mais antiga. Não inclua pendentes, não inclua caixinhas e não agrupe por categoria. Se não houver nenhum item no recorte, use text e diga isso de forma curta.",
      "Não faça um texto gigante. Mesmo quando a pergunta exigir contexto, entregue só o necessário para responder.",
    ];

    if (tom) regras.push("Tom/persona preferido para esta pessoa: " + tom);
    if (imersao) regras.push("Contexto pessoal disponível:\n" + imersao);
    regras.push("GLOSSÁRIO DAS CATEGORIAS:\n" + textoGlossarioCategorias());

    const historicoTexto = historicoSeguro.length
      ? historicoSeguro.map(function (m) {
          return (m.role === "user" ? "Usuário: " : "Assistente: ") + m.text;
        }).join("\n")
      : "Nenhuma conversa anterior nesta sessão.";

    const prompt = regras.join("\n\n") +
      "\n\nConversa anterior desta sessão:\n" + historicoTexto +
      "\n\nPergunta atual do usuário:\n" + perguntaLimpa +
      "\n\nResumo financeiro atual em JSON:\n" + JSON.stringify(resumo || {});
    ...
    const corpo = {
      ...
      generationConfig: {
        ...
        responseSchema: {
          type: "OBJECT",
          properties: {
            tipo: { type: "STRING", enum: ["text", "cards", "poll", "chart"] },
            texto: { type: "STRING" },
            cards: {
              type: "ARRAY",
              items: {
                ...
                properties: {
                  title: { type: "STRING" },
                  value: { type: "STRING" },
                  type: { type: "STRING", enum: ["ganho", "gasto", "guardado", "rendimento", "neutro"] },
    ...
    const texto = data.candidates && data.candidates[0] && data.candidates[0].content &&
      ...
      data.candidates[0].content.parts[0].text;
    if (!texto) return { ok: false, error: "O Gemini não retornou uma resposta." };

    let resposta;
    try {
      resposta = JSON.parse(texto);
    } catch (_err) {
      return { ok: false, error: "O Gemini não devolveu a resposta estruturada esperada." };
    }

    if (!resposta || ["text", "cards", "poll", "chart"].indexOf(resposta.tipo) < 0) {
      return { ok: false, error: "Formato de resposta da IA inválido." };
    }

    // No modo Davi, uma compra genérica não pode ser classificada como
    // alimentação ou gasto livre por chute. Se a pergunta fala de uma compra
    // com valor, mas não diz o que está sendo comprado, interrompemos a
    // resposta e pedimos a única informação que muda a regra de saldo:
    // alimentação x não alimentação. O clique da enquete volta como próxima
    // mensagem e o histórico preserva a pergunta original e o valor.
    const ehDavi = pessoaCodigo === "davi";
    const temValorCompra = /\bcompra\b/i.test(perguntaLimpa) && /(?:R\$|\d+(?:[.,]\d{1,2})?)/i.test(perguntaLimpa);
    const falaAlimentacao = /alimenta(?:ç|c)(?:ão|ao)|comida|comer|refei(?:ç|c)(?:ão|ao)|restaurante|lanch(?:e|onete)|mercado|supermercado|farm[aá]cia/i.test(perguntaLimpa);
    const falaNaoAlimentacao = /jogo|games?|roupa|vestu[aá]rio|eletr[oô]nico|celular|viagem|presente|assinatura|streaming|combust[ií]vel|transporte|carro|sapato|cal[cç]ado/i.test(perguntaLimpa);
    const perguntaCompraAmbiguaDavi = ehDavi && temValorCompra && !falaAlimentacao && !falaNaoAlimentacao;
    if (perguntaCompraAmbiguaDavi) {
      resposta = {
        tipo: "poll",
        texto: "",
        cards: [],
        chart: {},
        poll: {
          question: "Essa compra é de alimentação?",
          options: ["Sim, é alimentação", "Não, não é alimentação"]
        }
      };
    }

    // Para "transações recentes", a lista do app é a fonte de verdade e
    // vence qualquer formato que o modelo tenha escolhido. Assim a pergunta
    // sempre mostra 1 card por gasto pago/ganho recebido dos últimos 7 dias.
    const perguntaRecentes = /transa(?:ç|c)(?:ões|oes)|lan(?:ç|c)amentos|movimenta(?:ç|c)(?:ões|oes)/i.test(perguntaLimpa) &&
      /recent|últim|ultim|semana|dias|agora/i.test(perguntaLimpa);
    const recentes = resumo && Array.isArray(resumo.lancamentosRecebidosPagosUltimos7Dias)
      ? resumo.lancamentosRecebidosPagosUltimos7Dias
      : [];
    if (perguntaRecentes) {
      if (recentes.length) {
        resposta.tipo = "cards";
        resposta.cards = recentes.slice(0, 24).map(function (item) {
          const ehGanho = item.tipo === "ganho";
          const marcador = ehGanho ? "{{ganho:" : "{{gasto:";
          return {
            title: String(item.nome || (ehGanho ? "Ganho recebido" : "Gasto pago")),
            value: marcador + "R$ " + (Number(item.valor) || 0).toFixed(2).replace(".", ",") + "}}",
            type: ehGanho ? "ganho" : "gasto",
            subtitle: String(item.pessoa || ""),
            date: String(item.data || ""),
            category: String(item.categoria || ""),
            status: String(item.status || (ehGanho ? "recebido" : "pago"))
          };
        });
      } else {
        resposta.tipo = "text";
        resposta.texto = "Não houve gastos pagos nem ganhos recebidos nos últimos 7 dias.";
      }
    }

    // Sanitização mínima no backend: evita devolver listas enormes ou vazias.
    if (resposta.tipo === "text") {
      resposta.texto = String(resposta.texto || "").trim();
      const paragrafos = resposta.texto.split(/\n\s*\n/).filter(Boolean).slice(0, 3);
      resposta.texto = paragrafos.join("\n\n").slice(0, 1800);
      if (!resposta.texto) resposta.texto = "Não encontrei dados suficientes para responder com segurança.";
    }

    if (resposta.tipo === "cards") {
      resposta.cards = Array.isArray(resposta.cards) ? resposta.cards.slice(0, 24).map(function (c) {
        return {
          title: String(c.title || "Item").slice(0, 100),
          value: String(c.value || ""),
          type: ["ganho", "gasto", "guardado", "rendimento", "neutro"].indexOf(c.type) >= 0 ? c.type : "neutro",
          subtitle: String(c.subtitle || "").slice(0, 120),
          date: String(c.date || "").slice(0, 40),
          category: String(c.category || "").slice(0, 60),
          status: String(c.status || "").slice(0, 40)
        };
      }) : [];
      if (resposta.tipo === "cards" && !resposta.cards.length) {
        resposta.tipo = "text";
        resposta.texto = "Não encontrei itens suficientes para montar os cards.";
      }
    }

    if (resposta.tipo === "poll") {
      resposta.poll = resposta.poll || {};
      resposta.poll.question = String(resposta.poll.question || "Qual opção você prefere?");
      resposta.poll.options = Array.isArray(resposta.poll.options) ? resposta.poll.options.map(String).filter(Boolean).slice(0, 4) : [];
      if (resposta.poll.options.length < 2) {
        resposta.tipo = "text";
        resposta.texto = "Preciso de mais uma informação para responder com segurança.";
      }
    }

    if (resposta.tipo === "chart") {
      resposta.chart = resposta.chart || {};
      resposta.chart.title = String(resposta.chart.title || "Resumo");
      resposta.chart.subtitle = String(resposta.chart.subtitle || "");
      resposta.chart.segments = Array.isArray(resposta.chart.segments)
        ? resposta.chart.segments.map(function (seg) {
            return { label: String(seg.label || "Outros"), value: Number(seg.value) || 0 };
          }).filter(function (seg) { return seg.value > 0; }).slice(0, 10)
        : [];
      if (!resposta.chart.segments.length) {
        resposta.tipo = "text";
        resposta.texto = "Não encontrei dados suficientes para montar esse gráfico.";
      }
    }

    // Mantém "texto" para compatibilidade com versões antigas do frontend.
    return {
      ok: true,
      resposta: resposta,
      texto: resposta.tipo === "text" ? resposta.texto : "",
    };
  } catch (err) {
    return { ok: false, error: String(err) };
  }
...
function readCaixinhas(sheet) {
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return [];
  // Agora lê 7 colunas: P, Q, R, S, T, U, V
...
function saveCaixinhasBlock(sheet, rows) {
  const rowsToClear = linhasParaLimpar(sheet, rows);
  sheet.getRange(2, COL_GUARDADO, rowsToClear, 7).clearContent(); // Limpa P:V, incluindo prazo e ícone
  if (!rows || rows.length === 0) return;
