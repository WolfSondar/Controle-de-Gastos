...
function gerarChatComGemini(pessoa, pergunta, historico, resumo) {
  try {
    ...
    const regras = [
      ...
      "FORMATO chart: use para perguntas comparativas ou de ranking em que uma barra segmentada ajude, especialmente 'qual categoria gastei mais?', distribuição por categoria, participação dos gastos ou comparação de vários itens. O gráfico deve ser ENXUTO: title curto, sem texto explicativo e sem subtitle. Os segmentos devem conter somente dados que existam no resumo. Use label e value numérico exato; o frontend calcula as porcentagens e mostra a discriminação abaixo.",
