...
function gerarInsightComGemini(pessoa, periodo, resumo, opcoesModo) {
  try {
    ...
    const regrasComuns = [
      ...
      "Cada objeto deve ter exatamente estes campos: titulo, texto, recomendacao e tipo. O campo texto é a DICA: uma leitura curta e concreta do que os números mostram. O campo recomendacao é a AÇÃO: diga o que fazer a seguir, quando isso fizer sentido. NUNCA use a recomendacao para repetir o texto. NUNCA escreva instruções meta como 'não misture as datas', 'separar essa verba deixa mais claro', 'preste atenção' ou 'não confunda'; transforme isso em uma conclusão útil e uma ação concreta. Evite conselhos genéricos como 'gaste com sabedoria', 'organize suas finanças' ou 'tenha controle'. Se não houver uma ação realmente útil sustentada pelos dados, deixe recomendacao como uma frase curta e específica, não invente. O titulo deve ser CURTO, forte e contextual (1 a 3 palavras), em CAIXA ALTA, como \"GASTO\", \"RECEBIMENTO\", \"CAIXINHA\", \"RENDIMENTO\", \"COMPARAÇÃO\", \"ATENÇÃO\" ou outro título específico que combine com aquele insight. O campo tipo deve ser exatamente um destes valores: gasto, ganho, beneficio, guardado, rendimento, atencao, comparacao, planejamento ou geral. O tipo serve apenas para a interface aplicar a cor adequada ao titulo.",
      ...
      "Não use markdown, no máximo 1 emoji por insight. Cada item do array deve ser um objeto válido contendo somente titulo, texto, recomendacao e tipo; não coloque aspas extras, numeração, prefixos como 'Insight:' ou qualquer texto fora do array JSON.",
    ];
    ...
    const promptSistema = regrasComuns
      ...
      .concat(regrasTom)
      .concat(["Responda SOMENTE com o array JSON de " + QUANTIDADE_INSIGHTS_POR_PEDIDO + " objetos, nada além disso — sem crases, sem a palavra json antes. Cada objeto deve ter somente titulo, texto, recomendacao e tipo."])
      .join(" ");
