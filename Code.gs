 ...
 *   U = ICON (nome do arquivo do ícone personalizado, ex: caixa_zelda.png)
 *
...
const COL_ICONE = 21; // U — ícone personalizado da caixinha
...
function fecharMes(mes, ano) {
  ...
  const caixinhasProximoDavi = dadosDavi.caixinhas.map(function(c) {
    return { nome: c.nome, valorObjetivo: c.valorObjetivo, valorGuardado: valorTotalCaixinha(c), rendimentoTotal: 0, valorGuardadoMes: 0, icone: c.icone || "" };
  });
...
function readCaixinhas(sheet) {
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) return [];
  // Agora lê 6 colunas: P, Q, R, S, T, U
  const values = sheet.getRange(2, COL_GUARDADO, lastRow - 1, 6).getValues(); 
  const result = [];
  values.forEach(function (row) {
    if (row[0] !== "" && row[0] !== null) {
      result.push({
        ...
        icone: row[5] ? String(row[5]).trim() : "" // Coluna U
      });
    }
  });
  return result;
...
function saveCaixinhasBlock(sheet, rows) {
  const rowsToClear = linhasParaLimpar(sheet, rows);
  sheet.getRange(2, COL_GUARDADO, rowsToClear, 6).clearContent(); // Limpa P:U, incluindo o ícone
  if (!rows || rows.length === 0) return;
  const values = rows.map(function (r) {
    return [r.nome, r.valorObjetivo, r.valorGuardado, r.rendimentoTotal || 0, r.valorGuardadoMes || 0, r.icone || ""];
  });
  sheet.getRange(2, COL_GUARDADO, values.length, 6).setValues(values);
}
