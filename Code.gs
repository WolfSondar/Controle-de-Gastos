...
function gerarInsightComGemini(pessoa, periodo, resumo) {
  try {
    ...
    const regrasComuns = [
      ...
      "PADRÃO DE CORES OBRIGATÓRIO DO APP: ganhos vindos do Saldo normal são VERDE; ganhos identificados como Benefício são AZUL-CLARO; gastos fixos e variáveis são VERMELHO; dinheiro guardado em caixinhas é DOURADO; rendimento de caixinhas/investimentos é AZUL. Ao usar os marcadores de valores, respeite exatamente: {{ganho:...}} = verde somente para ganhos do Saldo, {{beneficio:...}} = azul-claro para Benefício, {{gasto:...}} = vermelho para despesas, {{guardado:...}} = dourado para caixinhas e {{rendimento:...}} = azul para rendimento. Nunca troque essas cores por julgamento de positivo/negativo." ,
      ...
      "Sempre que citar um valor em dinheiro, formate como reais no padrão brasileiro (vírgula decimal, sempre com 2 casas — ex: R$ 5,00 ou R$ 1.234,56) e marque TODO valor com chaves duplas indicando de que tipo ele é, pra cada um aparecer com a mesma cor usada no gráfico histórico do app (Ganhos do Saldo=verde, Benefício=azul-claro, Gastos=vermelho, Guardado=dourado, Rendimento=azul): {{ganho:R$ 5,00}} pra qualquer ganho recebido do Saldo normal; {{beneficio:R$ 5,00}} pra qualquer ganho recebido de Benefício; {{gasto:R$ 5,00}} pra qualquer valor de gasto/despesa (fixo, variável, de uma categoria, pendência a pagar); {{guardado:R$ 5,00}} pra valor ligado a caixinha — quanto já guardou, quanto falta pra bater a meta, o valor da própria meta; {{rendimento:R$ 5,00}} especificamente pro quanto uma caixinha/investimento rendeu. NUNCA use {{ganho:...}} para Benefício; use {{beneficio:...}}. Só use {{+R$ 5,00}} (favorável) ou {{-R$ 5,00}} (desfavorável) pro raro caso de um valor que não seja claramente nenhum dos quatro tipos, como um saldo geral. Exemplo real de frase: \"Você guardou {{guardado:R$ 150,00}} esse mês, seu Rendimento foi de {{rendimento:R$ 12,30}}, mas o Gasto com transporte subiu {{gasto:R$ 80,00}} em relação ao mês passado.\" NUNCA escreva um valor em reais sem um desses marcadores ao redor, e NUNCA deixe de indicar o tipo quando o valor claramente for um dos quatro — isso é o que importa mais, mais do que decidir se é bom ou ruim. PROIBIDO abreviar um valor monetário de qualquer forma (nunca escreva algo como \"R$ 3k\", \"R$ 2.5k\" ou \"3 mil reais\") — o valor dentro do marcador é sempre o número completo e exato, no formato R$ 0,00. Essa regra vale SEMPRE, mesmo que o tom/persona configurado pra essa pessoa seja informal, gamer ou de internet — a persona muda só o vocabulário ao redor do número, nunca o próprio número ou o marcador dele.",
